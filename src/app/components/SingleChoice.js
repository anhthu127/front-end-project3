...
import { RADIO, SINGLE_CHOICE, NUMBER, STRING } from '../config/common/TypeOfInput';
...
const SingleChoice = (props) => {
  ...
  let objSingleChoice = {
    ...
    input_type_id: SINGLE_CHOICE, // 4 là dạng multi select....
    question_choice: [],
    question_other: {}
  };
  ...
  const onClickSave = (e) => {
    ...
    rowLabel.forEach((e, i) => {
      ...
      question.type = RADIO;
      question.index = i;
      objSingleChoice.question_choice.push(question);
    });
    rowChild.forEach((e, i) => {
      ...
      objSingleChoice.row_child.push(objRowChild);
    });

    objSingleChoice.delete_choices = listDelete;
    objSingleChoice.question_other = questionOther;

    if (isHasChild) {
      objSingleChoice.isHasChild = 1;
    }
    if (preTitle !== "") {
      setTitle(preTitle);
      objSingleChoice.title = preTitle;
    }

    if (titleNote !== "") {
      setPreTitleNote(titleNote);
      objSingleChoice.title_note = titleNote;
    }
    props.getDataSection(objSingleChoice);
    props.onCancel();
  };
  ...
  const handleChangeUnitChildRow = (e, idxRow, idxChil) => {
    e = window.event || e;
    e.preventDefault();
    let rows = [...rowChild];
        ...
        (
          <div>
            ...
            {rowLabel.map((item, idx) => {
              return (
                <>
                  <div key={`single-${item.unique}-${item.id || ''}`}>
            ...
            <div style={{ paddingTop: "20px", paddingLeft: '15px' }}>
              <div><Checkbox onClick={e => onHandleChangeNote(e.target.value)} checked={isHasChild ? true : false} /> Câu hỏi có ghi chú?</div>
              ...
              }
              {
                isHasChild === 1 && rowChild.map((e, i) => {
                  return (
                    ...
                    // <p className="fontSize12">-{e.title}............................................................{e.unit}</p>
                  )
                })
              }
