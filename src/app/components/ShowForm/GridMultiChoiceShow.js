...
const GridMultiChoiceShow = (props) => {
  ...
  const onChange = (e, answerItem) => {
    let ansewerChange = answer.map(item => {
      if (item.row_id === answerItem.row_id && item.column_id === answerItem.column_id) {
  ...
  return (
    <>
      <Fragment key={`key-grid-multi-${props.data.id}-${props.data.unique}`}>
          ...
          <table>
            ...
            <tbody>
              {question_row.map((rowItem, indexRow) => {
                return (
                  <tr key={rowItem} className="border-table">
                    <td >{indexRow + 1}</td>
                    <td>{rowItem.title}</td>
                    {question_columns.map((columnItem, indexCol) => {
                      ...
                      return (
                        <td key={indexCol}>
                          <Checkbox disabled={!props.isEnable} checked={answerItem && answerItem.value ? true : false}
...
export default GridMultiChoiceShow;
